...
abstract class ModuleHolder {
    ...
    // A list of modules that have been added with the [use] method, but their dependenceies have not yet been registered
    ...
    private fun <T : GameModule> add(
        ...
        logger.debug("Registering module ${module::class.simpleName}")
    ...
    fun <T : GameModule> use(
        ...
        if (module.getDependencies().any { it.isInstance(module) }) {
            error("Tried to register module that depends on itself")
        }
    ...
    fun checkUnmetDependencies() {
        if (waiting.isNotEmpty()) {
            logger.debug(
                "The following dependencies are still waiting: {}",
                waiting.map { it.module::class.simpleName })
        }
        for ((module, filter, callback) in waiting) {
